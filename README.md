...
- 1970년 생일:타임스탬프가 1970년1월1일부터 현재까지의 초를 계산한 결과 값
- 1970년 1월1일 부터 16억 2천 ...초가 흐른시간이 현재시간 
...
- 폰트어썸(아이콘 웹폰트): 아이콘을 확대해도 꺠지지 않는다.(벡터이미지방식의 아이콘)
- 포토샵 디자인으로 아이콘을 만들면 확대시 깨짐(비트맵이미지=스칼라방식의 아이콘)
- 부트스트랩 그리드() 레이아웃 :화면을 12개의 컬럼으로 분리해서 크기를 반응형으로 만듭니다.
- row(가로줄),col(세로칸), .col-md-12 (화면가로 크기를 12개로 지정=100%로지정,조건은 md에서는 100%)
- 반응형의 화면크기 :xs엑스트라스몰(~750px),sm스몰(750px~),md미디어(970px~) ,lg라지(1170px~)
- 화면의 가로크기를 지정하는 이유(모바일, 테블릿,pc용 일떄 가로크기를 지정하기 위해서 입니다.)
...
- jsp(1세대-2000년-게시판)->서블렛(2세대-servelet- 2005년-게시판)->스트러츠(3세대 2010년대프레임워크)->스프링(4세대 2013년대,프레임워크)
