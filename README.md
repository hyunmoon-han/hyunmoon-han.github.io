...
#### 20210528(금) 작업.
- UI트렌드 용어:카드(트럼프) UI,뱃지(명찰),리본(이벤트표시)
- 소스 인스펙트(소스분석);
- 데이터를 전송할떄 사용 문장 = 쿼리스트링(QueryString)의 시작은 ?:  ?키-value&키=value
...
#### 20210527(목)작업.
- admin/board_list.html부터 시작--작업 ok
- 관리자단 adminLTE 적용-UI시간:(게시판CRUD -html,대시보드-html)
- 관리자단 기본 틀(템플릿): 공통 UI부분 작업ok.
- ctrl+k+(ctrl+)0 = 코드 축소(코드구조 확인)
- ctrl+K+(ctrl+)j = 코드 펼침
...
= html5내장된 유효성 검사? reauired(공백체크),type="email"(이메일형식체크) type="password"(입력값을 숨김처리), type="number"(숫자 체크)
- js 로 유효성검사? 비밀번호 확인(2개의 입력값을 체크)
- 반응형 서브페이지들(로그인, 회원가입, 마이페이지).
- HTML5부터는 유효성 검사가 내장되어있기 때문에 ,js로 빈칸인지 코딩할 필요가 없습니다.required 속성을 태그에 집어넣으면 끝.
- 기획시, 메인 css네이밍,서브1 css네이밍 
->과장/이사급이 UI를 2개파일 만듬 또는 터마포레스트 같은 디자인 UIHTML_css+js를 구매) ->기반으로 퍼블리셔 프론트 개발자가 css나 HTML작업을 이어서 합니다.
- 발주 금액이4000이상 2억 안팎인 프로젝트는 자체 UI사용.

#### 20210525(화) 작업.
- css: 계층:조부 > 부 > 본인(timnline) > 자식(collapse) > 손자(time-label) > 증손자
- board_view.html 댓글 AdminLTE3의 페이징 UI 추가.
